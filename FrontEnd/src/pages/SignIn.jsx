...
const SignIn = () => {
  const { t } = useTranslation(["signin", "register"]);
  ...
  /* =========================
     JWT NAME DECODER
  ========================= */
  const decodeNameFromToken = (token) => {
    try {
      const payload = token.split(".")[1];
      const base64 = payload.replace(/-/g, "+").replace(/_/g, "/");
      const parsed = JSON.parse(window.atob(base64));
      return parsed.name || parsed.username || parsed.email || null;
    } catch {
      return null;
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const name = decodeNameFromToken(text);
        ...
        setError(text || t("signin:invalid"));
      }
    } catch {
      ...
      <div className="bg-white p-8 rounded-xl shadow-md w-[420px] text-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-black text-white p-2 rounded hover:bg-gray-800 transition disabled:opacity-60"
          >
            {loading ? "Signing in..." : t("signin:submit")}
          </button>
        </form>
        ...
        <div className="relative flex items-center justify-center py-6">
          ...
          <span className="mx-4 text-gray-400 text-xs uppercase">
            {t("signin:or")}
          </span>
          <div className="flex-grow border-t border-gray-300" />
        </div>
